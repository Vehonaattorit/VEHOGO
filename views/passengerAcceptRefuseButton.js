...
const PassengerAcceptRefuseButton = (props) => {
  ...
  const getTripRoute = async (waypoints) => {
    try {
      ...
      const destination = workTrip.scheduledDrive.stops[workTrip.scheduledDrive.stops.length - 1].location
      var waypointsString = ''
      ...
      console.log('fetching the stuff', `https://maps.googleapis.com/maps/api/directions/json?origin=${origin.latitude},${origin.longitude}&destination=${destination.latitude},${destination.longitude}${waypointsString}&key=${googleMapsApiKey}`)
  ...
  const acceptPassenger = async () => {
    ...
    if (workTripToUpdate.scheduledDrive.stops.length > 2) {
      ...
      waypoints.forEach(waypoint => {
        ...
        waypoint.distanceFromOrigin = calculateDistance(firstStop.location.latitude, firstStop.location.longitude, waypoint.location.latitude, waypoint.location.longitude)
      })
      ...
      waypoints.push(workTripToUpdate.scheduledDrive.stops[workTripToUpdate.scheduledDrive.stops.length - 1])

      //update the stops to the data model before saving to firestore
      workTripToUpdate.scheduledDrive.stops = waypoints
    }
    else {
      workTripToUpdate.scheduledDrive.stops.splice(1, 0, new Stop({
        location: rideRequest.homeLocation,
        address: rideRequest.homeAddress,
        stopName: passengerUser.userName,
        userID: rideRequest.senderID,
